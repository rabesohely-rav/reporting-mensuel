...
function renderModalitesChart(rows) {
  ...
  chartModalites = new Chart($('chartModalites'), {
    type: 'pie',
    data: {
      labels: rows.map(r => wrapAxisLabel(r.label, 16)),
      datasets: [{
    ...
    options: buildPieOptions('bottom')
  });
}
...
function wrapAxisLabel(label, maxLen = 16) {
...
function applyAdaptivePageDensity(data) {
  const page1 = $('reportPage1');
  const maxTableRows = Math.max((data.communes_usagers || []).length, (data.permanences || []).length);
  page1.classList.remove('density-normal', 'density-dense', 'density-ultra');
  if (maxTableRows >= 22) {
    page1.classList.add('density-ultra');
  } else if (maxTableRows >= 18) {
    page1.classList.add('density-dense');
  } else {
    page1.classList.add('density-normal');
  }
}
  ...
  chartThematiques = new Chart($('chartThematiques'), {
    ...
    options: {
      ...
      scales: {
        ...
        x: { ticks: { autoSkip: false, maxRotation: 0, minRotation: 0, padding: 4, font: { size: 8.5 } }, grid: { display: false } }
      }
    }
  });
}
...
function prepareStablePrintFrame(callback) {
...
function fillForm(data) {
  ...
  $('storageSlot').value = d.storage_slot || defaultStorageSlot();
...
function init() {
  ...
  $('btnPrint').addEventListener('click', () => {
    ...
    renderReport(data);
    prepareStablePrintFrame(() => window.print());
  });
