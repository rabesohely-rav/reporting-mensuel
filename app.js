...
const SAMPLE = {
  ...
  storage_slot: 'ALZONNE - Janvier 2026',
  communes_usagers: [
...
function renderTableBody(tbodyId, rows) {
  const isYearTable = tbodyId === 'tbodyDemandesAnnee';
  $(tbodyId).innerHTML = rows.map(row => {
    const isZero = Number(row.value) === 0;
    const valueText = isYearTable && isZero ? '' : formatNumber(row.value);
    const valueClass = isYearTable && isZero ? ' class="zero-value" aria-label="0"' : '';
    return `
      <tr>
        <td>${escapeHtml(row.label || row.mois)}</td>
        <td${valueClass}>${valueText}</td>
      </tr>
    `;
  }).join('');
}
...
async function downloadPdf() {
  ...
  const filenameBase = (data.storage_slot || 'Reporting')
    .replace(/[\/:*?"<>|]+/g, ' ')
    .replace(/\s+/g, ' ')
    .trim() || 'Reporting';
